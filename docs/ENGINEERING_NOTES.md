...
au fil du développement pour pouvoir expliquer chaque ligne en entretien
(voir `CLAUDE.md`) — plus détaillé que le `README.md` principal, qui vise
une lecture rapide plutôt que l'exhaustivité.
...
| heading_rad | DOUBLE | cap global start→end (T-27), 0 = nord ; approximation à l'échelle du segment, pas tronçon par tronçon (T-07b jamais fait) |
| start_lat, start_lng | DOUBLE | position de référence pour interroger la météo (T-27) |
...
plutôt que CdA — le vent n'était pas encore branché dans cette
calibration au moment du fit (T-15 existait, pas intégré ici).
...
d'appareil connecté pour HRV/sommeil pour l'instant).
...
linéaire de quelques lignes, choix fait avec l'utilisateur avant de
coder. `temporal_cross_validate_ridge` découpe en blocs chronologiques
...
contrainte W' ≥ 0 est imposée explicitement là où elle a un sens (T-26,
...
de CP qu'au repos complet, pas de plancher.
...
dans le module : le vent est le facteur dominant d'un créneau à l'autre.
...
## Limites connues (toutes, en un seul endroit)
...
- Aucun segment n'a de profil tronçon par tronçon stocké (T-07b jamais
  fait) : la calibration, la comparaison de scénarios, la prévision de
  fenêtre et le pacing traitent tous un segment comme UN SEUL tronçon à
  pente/cap moyens — directionnellement correct depuis T-27, mais pas le
  vrai relief du segment.
- Le vent n'était pas branché au moment de la calibration CdA/Crr (T-17)
  ni de son backtest (T-18) — recalibrer avec le vent maintenant intégré
  (T-27) changerait probablement un peu le résultat, pas refait depuis.
- Le modèle de forme (T-24) ne généralise pas dans le temps (R² négatif
  en validation croisée temporelle) — diagnostiqué et documenté, pas
  corrigé : la cause probable (CP fixe comme référence de l'indice de
  performance, T-23) toucherait plusieurs tickets en cascade si changée.
- L'incertitude météo du Monte-Carlo (T-28) est une hypothèse assumée
  (±20% sur le vent), pas mesurée contre un historique prévision-vs-
  réalisé — le projet n'en a pas.
